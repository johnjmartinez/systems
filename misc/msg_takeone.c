/*
 *  simply request a message from a queue, and displays queue attributes.
 *
...
int main(int argc, char *argv[]) {
    ...
    msgq_id = mq_open(MSGQOBJ_NAME, O_RDWR);    /* opening the queue */
    if (msgq_id == (mqd_t)-1) {
        perror("In mq_open()");
        exit(1);
    }

    mq_getattr(msgq_id, &msgq_attr);            /* getting the attributes from the queue */
    printf("Queue \"%s\":\n\t- stores at most %ld messages\n\t-",  MSGQOBJ_NAME, msgq_attr.mq_maxmsg )
    printf("large at most %ld bytes each\n\t- currently holds %ld messages\n", 
        msgq_attr.mq_msgsize, msgq_attr.mq_curmsgs);

    msgsz = mq_receive(msgq_id, msgcontent, MAX_MSG_LEN, &sender); /* getting a message */
    if (msgsz == -1) {
